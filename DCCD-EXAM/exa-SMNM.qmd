...
author:
  - name: Ph.D. Candidate Sergio M. Nava Muñoz
    id: sn
    email: nava@cimat.mx
    affiliation: 
        - id: cimat
          ...
          city: Aguascalientes
          state: Ags
  - name: (Advisor) Ph.D. Mario Graff Guerrero
    id: mgg
    degrees: PhD
    email: mario.graff@infotec.edu.mx
    affiliation: 
      - name: CONAHCYT/INFOTEC
        city: Aguascalientes
        state: Ags
...
![Challenge](figs/Competencia.png){#fig-Challenge width="600" height="300"}
...
1. Review evaluation elements (metrics, statistical methods, configurations) to identify best practices and limitations.
2. Analyze key ML competitions to assess validation processes, transparency, and fairness.
3. Design advanced statistical evaluation schemes, validate through simulations or collaborations, and provide implementation guidelines.
:::

# Related Work {.smaller}

## Related work {  transition="slide"}
...
## Comparison of methods and tests used in ML evaluation frameworks {.smaller .scrollable transition="slide"  }
...
:  Comparison of methods and tests used in machine learning evaluation frameworks, highlighting their requirements and characteristics, including whether they require only predictions, allow multiple comparisons, use a single dataset, or rely on the competition's provided metric. {#tbl-LitReview2 .striped .hover .fixed-header-table}

::: footer
X: Indicates that the feature is present in the method or test.  
O: Indicates that the feature is not present in the method or test.
:::
# Theory framework
...
```{python}
#| echo: false
#| label: fig-f1VaxxStanceEUDensity
...
# Supongamos que 'perf' contiene los datos necesarios
import pandas as pd
import matplotlib.pyplot as plt

# Supongamos que 'perf' contiene los datos necesarios
df = pd.DataFrame(perf['samples']['calcular_f1_score_average=None'])
fig, ax = plt.subplots(1, 1)

# Lista para almacenar las leyendas de los puntos
point_labels = []

# Iterar sobre las columnas y trazar la densidad con el punto medio
for s in df.columns:
    # Graficar la densidad
    density_plot = df[s].plot(kind='density', bw_method=0.5, ax=ax)  
    # Calcular la media
    mean_value = df[s].mean()
    # Obtener el color de la línea actual
    line_color = density_plot.get_lines()[-1].get_color()
    # Añadir el punto en el valor medio
    ax.scatter(mean_value, 0, color=line_color, s=100)
    # Añadir la etiqueta del punto a la lista
    #point_labels.append(s)

# Añadir títulos y etiquetas
plt.title('Density plot of Macro-averaged F1 Score for "favor" and "against"')
plt.xlabel('Macro-averaged F1 Score')
plt.ylabel('Density')

# Crear una leyenda solo con los puntos
for i, label in enumerate(point_labels):
    ax.scatter([], [], color=density_plot.get_lines()[i].get_color(), s=100, label=label)
...
: Results for VaxxStance Close Track - Contextual (2021) for Basque and Spanish {#tbl-independent .striped .hover}
...
::: custom-small
| **Basque (WordUp.01)**  | LCI | Diff | UCI |   | **Spanish (WordUp.02)**  | LCI | Diff | UCI |
|----|----|----|----|----|----|----|----|----|
...
: Results for VaxxStance Close Track - Contextual (2021) for Basque and Spanish. Tha table contains IC of the difference between the top team and the other teams, for Basque (WordUp.01) and Spanish (WordUp.02) {#tbl-Paired .striped .hover}

![Paired Samples](figs/mejorordenado2.png){#fig-paired-samples width="600" height="300"}
:::

## Statistical Hypothesis Testing  {.smaller .scrollable transition="slide"}
...
: Macro-averaged F1 Scores for "favor" and "against" in the VaxxStance Close Track (2021) - Contextual Evaluation. {#tbl-VaxxStance3 .striped .hover}
::::
...
## Metrics used for competitions Comparison
...
: Metrics used for competitions Comparison. {#tbl-info-summary  .striped .hover}
:::
...
: Table Metrics and data considered across various competitions {#tbl-Competitions .striped .hover .fixed-header-table}
...
: Summary of competitions, subtasks, languages, ranking metrics, and participant details for key Natural Language Processing challenges. {#tbl-Results .striped .hover}
:::
### Table 2
...
::: custom-small
| Task                | Close Track-Textual (Spanish) | Close Track-Textual (Basque) | Close Track-Contextual (Spanish) | Close Track-Contextual (Basque) |
|---------------------|------------------------------|------------------------------|----------------------------------|----------------------------------|
...
:Comparison of performance metrics and statistical outcomes for the Close Track (Textual and Contextual) in Spanish and Basque, including number of participants ($m$), ties, and key statistical measures ($|win.-med|$, $CV$, and PPI). {#tbl-CompVaxxStance .striped .hover}
:::
...
::: panel-tabset
###  Single-Metric Performance Assessment
...
The performance metrics used are macro-averaged F1 score, accuracy, precision and reall. 
...
# Key Achievements of This Thesis

---



### Comparison of classifiers in challenge scheme (MCPR 2023)

- Part of the book series: Lecture Notes in Computer Science (LNCS, volume 13902).
- Included in the following conference series: Mexican Conference on Pattern Recognition (MCPR).
- [More details available at DOI: https://doi.org/10.1007/978-3-031-33783-3_9](https://doi.org/10.1007/978-3-031-33783-3_9)
- To propose tools that allow comparing the classification results of different competitors in a Challenge, in addition to performance metrics.


::: fragment
### Analysis of Systems' Performance in Natural Language Processing Competitions

(Special Section on “MCPR-IAPR Best Paper Award and MCPR-IAPR Student Paper Award”)


- Published in the journal **Pattern Recognition Letters**, focusing on contributions to natural language processing competitions.
- [More details available at DOI: https://doi.org/10.1016/j.patrec.2024.03.010](https://doi.org/10.1016/j.patrec.2024.03.010)
