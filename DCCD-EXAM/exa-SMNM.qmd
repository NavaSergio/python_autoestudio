...
Given the test dataset $x=x_1, \ldots , x_n$, in which $A$ beats $B$ by a magnitude $\delta(x)=\theta_A (x) - \theta_B (x)>0$
$$H_0:\theta_A \leq \theta_B;\;\;H_1:\theta_A > \theta_B$$

Estimate:
$$p-value(x)=p(\delta(X)>\delta(x)|H_0,x)$$
